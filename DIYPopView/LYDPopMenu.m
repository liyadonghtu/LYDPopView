...
#pragma mark - *******************************************************************
#pragma mark -- LYDMenuView 列表视图的声明
@interface LYDMenuView : UIView
...
#pragma mark - *******************************************************************
#pragma mark -- LYDMenuOverLay 背景视图声明
@interface LYDMenuOverlay : UIView

@end

#pragma mark -- LYDPopMenu popMenu内部声明
@interface LYDPopMenu ()

@property (nonatomic, strong) LYDMenuView *menuView;
...
#pragma mark -- LYDMenuOverLay 背景视图实现
@implementation LYDMenuOverlay
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touched == self) {
        ...
        for (UIView *v in self.subviews) {
            if ([v isKindOfClass:[LYDMenuView class]]
...
#pragma mark - *******************************************************************
#pragma mark -- LYDMenuItem 一个选项模型的实现
@implementation LYDMenuItem
...
+ (instancetype) menuItem:(NSString *) title
                   ...
                   action:(SEL) action
{
    return [[LYDMenuItem alloc] init:title
                              image:image
                             target:target
                             action:action];
}
...
#pragma mark - *******************************************************************
#pragma mark -- LYDMenuView 列表视图的实现
typedef enum {
             ...
             menuItems:(NSArray *)menuItems
{
    ...
    LYDMenuOverlay *overlay = [[LYDMenuOverlay alloc] initWithFrame:view.bounds];
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:0.2
                             animations:^(void) {
                                 ...
                                 if ([self.superview isKindOfClass:[LYDMenuOverlay class]])
...
- (void)performAction:(id)sender
{
    ...
    LYDMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView
{
    ...
    UIFont *titleFont = [LYDPopMenu titleFont];
    ...
    for (LYDMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [LYDMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [LYDMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
             ...
             inContext:(CGContextRef) context
{
    ...
    UIColor *tintColor = [LYDPopMenu tintColor];
    if (tintColor) {
...
#pragma mark - *******************************************************************
#pragma mark -- LYDPopMenu pop菜单视图的最终实现
static LYDPopMenu *gMenu;
static UIColor *gTintColor;
static UIFont *gTitleFont;
static CGFloat gCornerRadious;


@implementation LYDPopMenu
              ...
              menuItems:(NSArray *)menuItems
{
    ...
    self.menuView = [[LYDMenuView alloc] init];
    self.menuView.cornerRadius = gCornerRadious;
...
+ (CGFloat)cornerRadious {
    return gCornerRadious;
}

+ (void)setCornerRadious:(CGFloat)cornerRadious {
    if (cornerRadious != gCornerRadious) {
        gCornerRadious = cornerRadious;
    }
}
